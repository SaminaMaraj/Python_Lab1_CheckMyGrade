...
class TestCheckMyGrade(unittest.TestCase):
    ...
    def test_persistence_load_and_search_timing(self):
        try:
            storage.add_course(Course("DATA200", "Data Science", "Intro", 3))
        except Exception:
            pass

        results_now, _ = storage.search_students(lambda r: r["Course_id"].lower() == "data200")
        if not results_now:
            storage.add_student(Student("samina@gmail.com", "Samina", "User", "DATA200", "A", 95.0))
        results, t = storage.search_students(lambda r: r["Course_id"].lower() == "data200")
        print(f"Persistence search time: {t:.6f}s (found {len(results)} record(s))")
        self.assertGreaterEqual(len(results), 1)
